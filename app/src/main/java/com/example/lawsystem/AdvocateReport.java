...
import com.example.lawsystem.databinding.ActivityAdvocateReportBinding;
...
public class AdvocateReport extends AppCompatActivity {
    private ActivityAdvocateReportBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding=ActivityAdvocateReportBinding.inflate(getLayoutInflater());
        setContentView(binding.getRoot());
        binding.advreport.setLayoutManager(new LinearLayoutManager(this));
        advreport=new AdvocateReportAdapter(this, list, false);
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                for (DataSnapshot snapshot1:snapshot.getChildren()){
                    for(DataSnapshot snapshot2 : snapshot1.getChildren()){
                        if(snapshot2.getKey().equals(username)){
                            for(DataSnapshot snapshot3: snapshot2.getChildren()){
                                ChatModel model = snapshot3.getValue(ChatModel.class);
                                if(model.isPayment()){
                                    list.add(model);
                                }
                            }
